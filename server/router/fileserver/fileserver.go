...
func (s *FileServerService) RegisterRoutes(echoServer *echo.Echo) {
	...
	// Serve Geek Mode static files
	// Allows remote verification of artifacts generated by Geek Agent
	fileGroup.GET("/geek/:userID/*", s.serveGeekFile)
}
...
// serveGeekFile serves static files generated by Geek Agent.
// Path: /file/geek/:userID/*
func (s *FileServerService) serveGeekFile(c echo.Context) error {
	ctx := c.Request().Context()
	requestedUserIDStr := c.Param("userID")
	filePath := c.Param("*")

	// 1. Authentication & Authorization
	// Get current user to verify they match the requested userID
	user, err := s.getCurrentUser(ctx, c)
	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to authenticate").SetInternal(err)
	}
	if user == nil {
		return echo.NewHTTPError(http.StatusUnauthorized, "unauthorized")
	}

	requestedUserID, err := util.ConvertStringToInt32(requestedUserIDStr)
	if err != nil {
		return echo.NewHTTPError(http.StatusBadRequest, "invalid user ID")
	}

	// Strict check: Users can only see their own geek files
	if user.ID != requestedUserID {
		return echo.NewHTTPError(http.StatusForbidden, "forbidden: cannot access other user's workspace")
	}

	// 2. Resolve Path
	// Must match the path used in GeekParrot (HOME/.divinesense/claude/user_{id})
	homeDir, err := os.UserHomeDir()
	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to get home directory")
	}

	baseDir := filepath.Join(homeDir, ".divinesense", "claude", fmt.Sprintf("user_%d", requestedUserID))
	fullPath := filepath.Join(baseDir, filePath)

	// 3. Security: Prevent Directory Traversal
	// Clean the path and ensure it starts with baseDir
	...
	// 5. Serve File
	// Disable caching for these files as they are likely being iterated on
...
func (s *FileServerService) getOrGenerateThumbnail(_ context.Context, attachment *store.Attachment) ([]byte, error) {
