...
- **Components**: PostgreSQL container + Backend dev server + Frontend dev server
- **Command**: `make start`
- **Ports**: Backend 28081, Frontend 25173, PostgreSQL 25432

### Production (Docker Mode)
- **Purpose**: Single-server production deployment with Docker containers
- **Components**: PostgreSQL container + DivineSense container
- **Installation**: `deploy/aliyun/install.sh --mode=docker` (default)
- **Management**: `cd /opt/divinesense && ./deploy.sh <command>`

### Production (Binary Mode) - Recommended for Geek Mode
- **Purpose**: Production deployment with native systemd service
- **Components**: Systemd service + PostgreSQL (Docker or system)
- **Installation**: `deploy/aliyun/install.sh --mode=binary`
- **Management**: `/opt/divinesense/deploy-binary.sh <command>`
- **Advantages**:
  - Native Claude Code CLI integration (Geek Mode)
  - Faster startup, lower overhead
  - Easier updates (binary replacement with checksum verification)
- **Documentation**: [Deployment Guide](../deployment/BINARY_DEPLOYMENT.md)
...
# Binary mode (Geek Mode recommended)
curl -fsSL https://raw.githubusercontent.com/hrygo/divinesense/main/deploy/aliyun/install.sh | sudo bash -s -- --mode=binary
```

**Directory Structure (Binary Mode):**
```
/opt/divinesense/          # Installation root
├── bin/                   # Binary
│   └── divinesense
├── data/                  # Working directory (Geek Mode)
├── logs/                  # Logs
├── backups/               # Database backups
├── docker/                # PostgreSQL Docker config (optional)
│   ├── postgres.yml
│   └── .env
└── deploy-binary.sh      # Management script

/etc/divinesense/          # Configuration
└── config                 # Environment variables
└── .db_password          # Database password (600权限)
...
**Geek Mode (Optional - Claude Code CLI integration):**
```bash
# Enable Geek Mode for code-related tasks
