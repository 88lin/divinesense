...
test_direct_env_var() {
    unset DIVINESENSE_DSN DIVINESENSE_DSN_FILE
    export DIVINESENSE_DSN="direct_value"

    result=$("$SCRIPT_DIR/entrypoint.sh" sh -c 'echo $DIVINESENSE_DSN' 2>&1)
    if [ "$result" = "direct_value" ]; then
        pass "Direct env var works"
    else
        fail "Direct env var: expected 'direct_value', got '$result'"
    fi
    unset DIVINESENSE_DSN
}
...
test_file_env_var_readable() {
    ...
    export DIVINESENSE_DSN_FILE="$TEMP_DIR/dsn_file"
    ...
    unset DIVINESENSE_DSN_FILE
}

# Test 3: Error when file doesn't exist
test_file_env_var_missing() {
    unset DIVINESENSE_DSN DIVINESENSE_DSN_FILE
    export DIVINESENSE_DSN_FILE="$TEMP_DIR/nonexistent_file"

    if result=$("$SCRIPT_DIR/entrypoint.sh" sh -c 'echo $DIVINESENSE_DSN' 2>&1); then
...
test_file_env_var_unreadable() {
    ...
    export DIVINESENSE_DSN_FILE="$TEMP_DIR/unreadable_file"
